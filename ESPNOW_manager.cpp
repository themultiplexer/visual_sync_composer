...
uint8_t old_raw_bytes[LEN_RAWBYTES_MAX];
...
void* ESPNOW_manager::sock_recv_thread (void *p_arg)
{
    ...
    {	
        ...
        {
			for (int i = 0; i < raw_bytes_len; i++) {
				if (raw_bytes[i] != old_raw_bytes[i]) {
					printf("\033[22;34m%02hhX\033[0m ", raw_bytes[i]);
				} else {
			...
			printf("\n");
			memcpy(old_raw_bytes, raw_bytes, raw_bytes_len);
